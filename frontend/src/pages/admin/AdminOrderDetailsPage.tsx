...
import { AgentResponse, OrderResponse, TrackingResponse } from '../../types';
...
export const AdminOrderDetailsPage: React.FC = () => {
  ...
  const fetchDetails = async (showLoadingState = true) => {
    ...
    try {
      const [orderData, eventsData, agentsData] = await Promise.all([
  ...
  if (isLoading) {
    return (
      <div className="min-h-[50vh] flex items-center justify-center">
        <Spinner size="lg" label="Loading admin order details..." />
      </div>
    );
  }
    ...
    <div className="space-y-6 max-w-5xl mx-auto">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 space-y-6">
          ...
          <Card>
            <CardHeader
              title="Consignment & Delivery Addresses"
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-xs">
              ...
              <div className="p-3.5 bg-slate-50 rounded-xl border border-slate-200/80 space-y-1">
                ...
                <p className="text-slate-500 text-[11px]">
                  Zone IDs: Pickup ({order.pickup_zone_id?.substring(0, 8) || 'None'}), Delivery ({order.delivery_zone_id?.substring(0, 8) || 'None'})
                </p>
              </div>

              <div className="sm:col-span-2 p-3.5 bg-slate-50 rounded-xl border border-slate-200/80 space-y-1">
                <span className="text-[10px] font-bold uppercase text-slate-400 block">Origin Pickup Address</span>
                <p className="text-slate-800 leading-relaxed">{order.pickup_address}</p>
              </div>

              <div className="sm:col-span-2 p-3.5 bg-slate-50 rounded-xl border border-slate-200/80 space-y-1">
                <span className="text-[10px] font-bold uppercase text-slate-400 block">Destination Delivery Address</span>
